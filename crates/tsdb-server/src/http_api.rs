...
fn handle_http_request(
    ...
    let response = match (method.as_str(), path.as_str()) {
        ...
        ("POST", p) if p.starts_with("/api/v1/write") => match handle_write(&body, db) {
        ...
        ("GET", p) if p.starts_with("/api/v1/timeseries") => match handle_timeseries(&body) {
            Ok(svg) => http_response_svg(200, &svg),
            Err(e) => http_response(500, &format!(r#"{{"error":"{}"}}"#, e)),
        },
        ("POST", p) if p.starts_with("/api/v1/timeseries") => match handle_timeseries(&body) {
            Ok(svg) => http_response_svg(200, &svg),
            Err(e) => http_response(500, &format!(r#"{{"error":"{}"}}"#, e)),
        },
        ...
        ("GET", "/api/v1/dashboard/performance") => match handle_performance_dashboard() {
            Ok(html) => http_response_html(200, &html),
            Err(e) => http_response(500, &format!(r#"{{"error":"{}"}}"#, e)),
        },
...
fn handle_write(body: &str, db: &StorageEngine) -> Result<()> {
    ...
    let timestamp = write_req["timestamp"]
        .as_i64()
...
fn handle_business_dashboard(
    ...
    Ok(tsdb_dashboard::DashboardRenderer::render_business_html(
        &dash,
    ))
}
...
fn handle_timeseries(body: &str) -> Result<String> {
    ...
    let data_dir = std::env::var("TSDB_DATA_DIR").unwrap_or_else(|_| "./data".to_string());
...
fn handle_performance_dashboard() -> Result<String> {
    ...
    Ok(tsdb_dashboard::DashboardRenderer::render_performance_html(
        &dash,
    ))
}
